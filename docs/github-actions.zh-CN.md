...
**触发条件：** push 提交消息或 PR 标题包含 `[dify-upgrade]` 时运行，或手动触发  
**依赖：** `validate`  
**超时：** 10 分钟  
...
| 步骤 | 说明 |
|---|---|
| 验证 DIFY_VERSION | 检查 `DIFY_VERSION` 文件是否与实际子模块 SHA 一致；不一致时以退出码 1 失败 |
| 版本范围 + 契约测试 | `bash scripts/check-dify-compat.sh` — 运行版本范围检查及对真实 Postgres 的契约测试 |

**触发方式：** 更新 Dify 子模块时，在 push 提交消息（或 PR 标题）中包含 `[dify-upgrade]`。
...
| 触发事件 | validate | dify-compat | test-unit | security | build | deploy-staging | deploy-production |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| PR → main | ✓ | — | ✓ | ✓ | — | — | — |
